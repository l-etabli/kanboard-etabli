 ...
 */
class MyFormHelper extends Base
{
    /**
    ...
    public function input($type, $name, $values = array(), array $errors = array(), array $attributes = array(), $class = '')
    {
        ...
        $html .= $this->errorList($errors, $name, $class);
     ...
     * @return string
     */
    private function errorList(array $errors, $name, $class = '')
    {
        $html = '';
        if (isset($errors[$name])) {
            ...
            foreach ($errors[$name] as $error) {
                $html .= '<li class="'.$class.'">'.$this->helper->text->e($error).'</li>';
            }
